...
- **Le parcours — 67 jours (26 mai → 31 juillet)** : une leçon complète par jour, datée et numérotée (vocabulaire → grammaire → conjugaison → dialogue → prononciation → quiz → bilan). Mai (26-31) = mise en route en douceur, juin = les bases (1-10) puis pratique & révision (11-30), **juillet = vitesse supérieure** (heure & routine, présent continu, tout le passé, les trois futurs, comparatif/superlatif, puis l’anglais pratique et pro : directions, boutiques, téléphone, e-mail, pitch métier et visite immobilière complète — voir `src/data/programme-juillet.js`). Les jours se débloquent les uns après les autres **et seulement le vrai jour** : on ne peut pas faire le 27 avant le 27 (verrou calendaire, départ le 26 mai 2026).
- **Quiz de révision enrichis (juillet)** : à partir du jour 37, les quiz quotidiens mélangent aux 4 types classiques des **questions d’écoute, des dictées et des QCM** (audio par synthèse vocale) — voir `src/data/revision-juillet.js`.
- **Entraînement libre** : flashcards de vocabulaire (decks Juin, **Juillet** et Immobilier), quiz de révision par jour, et surtout **l’atelier d’exercices** (`/session/exercices`) — un grand catalogue de tous types (compréhension écrite, compréhension orale, expression orale, dictées, exercices mélangés), classé en **5 niveaux** de difficulté (dont le niveau **Expert ☀️** dédié à juillet : récit d’agence au passé, e-mail client, script téléphonique, dictée pro…). Chaque exercice se débloque quand le **jour requis** du parcours est terminé. Contenu dans `src/data/exercices.js`.
